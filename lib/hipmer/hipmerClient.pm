...
sub new
{
    ...
    my $self = {
	client => hipmer::hipmerClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
=head2 run_hipmer_hpc

  $output = $obj->run_hipmer_hpc($params)
...
<pre>
$params is a hipmer.AssemblyParams
$output is a hipmer.AssemblyOutput
...
# Authentication: required
	...
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function run_hipmer_hpc (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        ...
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to run_hipmer_hpc:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'run_hipmer_hpc');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "hipmer.run_hipmer_hpc",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method run_hipmer_hpc",
...
sub version {
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method run_hipmer_hpc",
            status_line => $self->{client}->status_line,
            method_name => 'run_hipmer_hpc',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for hipmer::hipmerClient\n";
    }
    if ($sMajor == 0) {
        warn "hipmer::hipmerClient version is $svr_version. API subject to change.\n";
    }
}
...
package hipmer::hipmerClient::RpcClient;
